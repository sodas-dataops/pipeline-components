...
if __name__ == '__main__' :
    ...
    # 임시 파일 경로 설정
    local_file_path = './tmp/mysql_result.csv'
    report_file_path = './tmp/task_report.md'
    os.makedirs(os.path.dirname(local_file_path), exist_ok=True)

    # MySQL에서 SQL 쿼리 실행 및 CSV 저장
    mysql_config = args['settings']
    sql_query = args['settings']['sql_query']
    
    # 설정에서 chunksize 가져오기 (기본값: 5000)
    chunksize = args['settings'].get('chunksize', 5000)
