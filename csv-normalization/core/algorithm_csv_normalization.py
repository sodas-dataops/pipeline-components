...
def generate_report(input_data: pd.DataFrame, output_data: pd.DataFrame, 
    ...
    """
    normalization_method = settings.get('normalization_method', 'z_score') if settings else 'z_score'
    ...
    method_info = f"- **정규화 방법**: {normalization_method}"
    range_info = f"- **특성 범위**: {feature_range}" if normalization_method == 'min_max' else ""
    
    report = f"""# CSV 데이터 정규화 작업 보고서

## 1. 작업 개요
- **작업 유형**: CSV 데이터 정규화
- **실행 시간**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
- **소요 시간**: {elapsed_time:.2f}초
- **정규화된 컬럼**: {len(target_columns) if target_columns else '전체 숫자형 컬럼'}
{columns_info}
{method_info}
{range_info}
...
- **정규화 효율**: {len(input_data) / elapsed_time:.2f} 행/초

## 5. 작업 상태
- **상태**: 성공
- **데이터 정규화**: 데이터 정규화가 성공적으로 완료됨
...
def solution(input_data: StringIO, output_filename: str, settings: dict = None):
    ...
    - settings (dict): 정규화 설정
        - normalization_method (str): 정규화 방법 ('z_score', 'min_max', 'robust')
        - target_columns (list): 정규화할 컬럼 목록
        - feature_range (list): Min-Max 정규화 범위 [min, max]
    
    Returns:
    - tuple: (저장된 파일 경로, 보고서 내용)
    """
    start_time = time.time()
    print(f"\n[시작] 데이터 정규화 작업을 시작합니다.")
    
    # 설정 기본값 설정
    if settings is None:
        settings = {}
    
    normalization_method = settings.get('normalization_method', 'z_score')
    target_columns = settings.get('target_columns', [])
    feature_range = settings.get('feature_range', [0, 1])
    
    print(f"- 정규화 방법: {normalization_method}")
    if target_columns:
    ...
    if not columns_to_process:
        raise ValueError("정규화할 숫자형 컬럼이 없습니다.")
    
    print(f"- 정규화 대상 컬럼: {', '.join(columns_to_process)}")
    ...
    try:
        ...
        for col in columns_to_process:
            if normalized_df[col].isnull().any():
                print(f"- 경고: 컬럼 '{col}'에 결측값이 있습니다. 정규화 전에 결측값을 처리해주세요.")
        
        if normalization_method == 'z_score':
            # Z-score 정규화 (평균 0, 표준편차 1)
            scaler = StandardScaler()
            normalized_df[columns_to_process] = scaler.fit_transform(normalized_df[columns_to_process])
            print(f"- Z-score 정규화 완료: 평균=0, 표준편차=1")
            
        elif normalization_method == 'min_max':
            # Min-Max 정규화 (0-1 범위)
            scaler = MinMaxScaler(feature_range=tuple(feature_range))
            normalized_df[columns_to_process] = scaler.fit_transform(normalized_df[columns_to_process])
            print(f"- Min-Max 정규화 완료: 범위={feature_range}")
            
        elif normalization_method == 'robust':
            # Robust 정규화 (중앙값과 IQR 기반)
            scaler = RobustScaler()
            normalized_df[columns_to_process] = scaler.fit_transform(normalized_df[columns_to_process])
            print(f"- Robust 정규화 완료: 중앙값=0, IQR=1")
            
        else:
            raise ValueError(f"지원하지 않는 정규화 방법: {normalization_method}")
    ...
    print(f"\n[4/4] 결과를 저장합니다...")
    try:
        normalized_df.to_csv(output_filename, index=False, encoding='utf-8')
    ...
    print(f"- 정규화된 데이터: {len(normalized_df):,}행")
    print(f"- 정규화된 컬럼: {len(columns_to_process)}개")
    print(f"- 소요 시간: {elapsed_time:.2f}초")
    
    # 보고서 생성
    report = generate_report(df, normalized_df, output_filename, 
