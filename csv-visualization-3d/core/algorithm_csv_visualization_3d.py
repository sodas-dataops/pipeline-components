...
def create_image_3d(
    ...
    """
    matplotlib을 사용하여 이미지 3D 산점도를 생성하는 함수.
    ...
    # 3D 산점도 생성
    fig = plt.figure(figsize=(fig_size['width'], fig_size['height']))
    ...
    if color_column and color_column in df.columns:
        ...
        # 기본 3D 산점도
        ax.scatter(df[x_column], df[y_column], df[z_column], alpha=alpha, s=s)
    
    # 축 라벨 설정
    xlabel = common_params.get('xlabel', x_column)
    ylabel = common_params.get('ylabel', y_column)
    zlabel = common_params.get('zlabel', z_column)
    ...
    # 차트 제목 설정
    title = common_params.get('title', '3D Scatter Plot')
...
def create_interactive_3d(
    ...
    """
    plotly를 사용하여 인터랙티브 3D 산점도를 생성하는 함수.
    ...
    fig.update_layout(
        ...
        height=interactive_params.get('height', 800),
        scene=dict(
            xaxis_title=common_params.get('xlabel', x_column),
            yaxis_title=common_params.get('ylabel', y_column),
            zaxis_title=common_params.get('zlabel', z_column),
            bgcolor=interactive_params.get('scene_bgcolor', 'white'),
            xaxis=dict(backgroundcolor=interactive_params.get('axis_bgcolor', 'white')),
            yaxis=dict(backgroundcolor=interactive_params.get('axis_bgcolor', 'white')),
            zaxis=dict(backgroundcolor=interactive_params.get('axis_bgcolor', 'white'))
        ),
    ...
    fig.update_traces(
        marker=dict(
            size=interactive_params.get('point_size', 5),
...
def generate_report(
    ...
    z_stats = df[z_column].describe()
    
    # 상관관계 계산
    xy_corr = df[x_column].corr(df[y_column])
    xz_corr = df[x_column].corr(df[z_column])
    yz_corr = df[y_column].corr(df[z_column])
    ...
    report = f"""# 3D 산점도 생성 작업 보고서

## 1. 작업 개요
- **작업 유형**: {chart_type.title()} 3D 산점도 생성
...
## 4. 디자인 파라미터
- **제목**: {design_params.get('common', {}).get('title', '3D Scatter Plot')}
- **X축 라벨**: {design_params.get('common', {}).get('xlabel', x_column)}
- **Y축 라벨**: {design_params.get('common', {}).get('ylabel', y_column)}
- **Z축 라벨**: {design_params.get('common', {}).get('zlabel', z_column)}
...
## 7. 작업 상태
- **상태**: 성공
- **처리 결과**: {chart_type.title()} 3D 산점도가 성공적으로 생성됨
"""
    return report
...
def solution(
    ...
    if z_column not in dataFile.columns:
        raise ValueError(f"Z축 컬럼 '{z_column}'이 데이터에 존재하지 않습니다.")
    ...
    if chart_mode == 'image':
        ...
        create_image_3d(dataFile, x_column, y_column, z_column, color_column, size_column, actual_output_file, design_params)
        chart_type = 'image'
        ...
        create_interactive_3d(dataFile, x_column, y_column, z_column, color_column, size_column, actual_output_file, design_params)
